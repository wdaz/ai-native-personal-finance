...
| `secret-scan.sh`       | The two scans with their flags: `history` (the changes in every commit, merges included, and every commit and annotated-tag message — CI and `npm run secrets:scan`) and `staged` (the pre-commit hook, which cannot see the message being written); both ignore an inline `gitleaks:allow` comment |
