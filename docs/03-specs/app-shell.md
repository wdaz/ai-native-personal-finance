...
## 6. Interfaces
### UI
`src/ui/Shell`, `Sidebar`, `BottomNav`, `NavItem`, `PageHeader`, `ResetBanner`, `AgentToolsStatus` (SPEC-webmcp-tools), `LogoutButton` (SPEC-auth). Icons from `src/ui/icons` (house, arrows-down-up, chart-donut, jar-fill, receipt, arrow-fat-lines-left, sign-out).
### API
