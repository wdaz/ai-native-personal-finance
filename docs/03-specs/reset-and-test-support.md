...
always resets. ADR-0007 carries the matching dated amendment. v1.3 (2026-09-23, owner decision at the T-08 plan gate, Q1 (a): "tövsiyə olunanlarla
...
| API | `/api/admin/reset` 204/400/401 and the `ResetLog` row's reason — `POST` without a body `manual`, with one its `reason`, `GET` with either secret `scheduled` once the interval has passed, 200 `{ reset: false, dueAt }` before it (v1.4); a refused request writes no row; `/api/test/seed` each variant produces the documented shape (in the database; via `/api/overview` from T-09); session rejected after reset (US-03 AC3) | US-36, US-37 AC1, US-03 AC3 |
