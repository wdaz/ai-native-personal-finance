...
pre-commit hook with 16 `postgres_connection_string` findings, all in this file: the fake URLs of
its prose and of the test code it quotes (`user:pw@…`). The same URLs in the new test files
(`tests/unit/shared/env.test.ts`, `database-guard.test.ts`, three more) would have stopped their
commit too, which a staged scan over every scratch file confirmed before they were rewritten. The
rule (`.gitleaks.toml`) exempts a host of `localhost` or
