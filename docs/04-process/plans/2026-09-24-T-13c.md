...
Status: **Done** — executed as T-13c and merged (PR #39, 2026-09-25, merge `8ffe0b1`) (v0.3 —
2026-09-25: the owner's go-ahead came, "start, subagent-driven", and the plan was executed; the
text below is the plan as it stood when it was handed over, and is not rewritten — what the
execution measured differently (five numbers), the code it changed (TD-10's URL check) and the
deviations are in the process log's T-13c execution entry; v0.2 — 2026-09-25: the answers are
recorded and Task 3 is rewritten for Q4 = a linter; v0.1 — 2026-09-24) · Author(s): Agent
(Claude Code) · Date: 2026-09-24
...
`4969d1f` (PR #36's merge); `origin/main` is `910ad2d` on 2026-09-25 (PR #38 changed
`scripts/traceability.ts` and the process log only — nothing this plan touches)
...
> it.** The owner's answers are in "Questions and the owner's answers"; what was still open at v0.2
> was the go-ahead ("start") and the execution method, which came on 2026-09-25 (v0.3, see Status).
>
...
> scratch verification `build-workflow.md` §2 (v1.2) allows, on 2026-09-24 and again on 2026-09-25
> for Task 3's rewrite. Nothing is committed but this file, no other tracked file stays changed
> (the second round also edited and restored `package.json` and `package-lock.json`), and the list
> of what ran is finding F9. Two things outside the tree: a scratch database
> `personal_finance_t13c` in the running Docker Postgres (dropped on 2026-09-24) and a git-ignored
> `.env.local` and `node_modules` in the worktree (deleted with it).
...
the login walkthrough's comment is true (TD-8), a bare `fr` grid track fails `npm run lint`
and the unit suite (TD-9, Stylelint), `APP_ENV=test` and the destructive database commands refuse to run where a real database
...
TD-9 is one Stylelint rule (`stylelint.config.mjs`, `npm run lint:css`, chained into `npm run
lint`) with a violation fixture and a control that a unit test hands to the linter. TD-10
...
npm 11.19 (`engines`: node >= 26, npm >= 11.19), **Stylelint 17.15.0 (new dev dependency, Q4)**.
No other new dependency — the font package is only fetched with `npm pack` into a scratch
directory.
...
- "`npm audit --audit-level=high` must stay at 0" and `.npmrc`'s `strict-allow-scripts=true` stays
  as it is. **One dev dependency is added — `stylelint` 17.15.0 — by the owner's answer to Q4**;
  measured with it: 75 packages added, `npm audit` 0 vulnerabilities, `install-scripts.test.ts`
  (the policy test) passing. Nothing else is added.
...
5. **A multi-digit or decimal `fr`, and a stylesheet under `app/(app)/`** — `minmax(0, 11fr)` must
   pass and `1.5fr` must fail (a regex that starts matching at the second digit reads `11fr` as a
   bare `1fr`; dropping the first lookbehind made the control fixture fail, measured), and the
   glob must reach `app/(app)/overview/page.module.css`, a directory named with parentheses.
   `minmax(auto, 1fr)` must fail (it is the bare shorthand's expansion). Task 3,
   `tests/fixtures/css-grid/` and the real-tree test.
...
provably definite. The rule is Stylelint's `declaration-property-value-disallowed-list` on
`grid-template-columns` with one regular expression (a number and `fr` that is not the maximum of
`minmax(<definite>, …)`, matched from the number's first digit); Stylelint parses the CSS, so a
comment that spells out `1fr` (`page.module.css`'s header does) is not a declaration and needs no
special case. It reads `.css` files under `app/` and `src/` (a superset of the `*.module.css` the
entry names — `globals.css` could grow a grid too), the value of `grid-template-columns` only;
`grid-template-rows`, `grid-auto-columns` and the `grid` / `grid-template` shorthands are not read,
and nothing uses them. Measured: `npx stylelint "app/**/*.css" "src/**/*.css"` exits 0 on today's
tree; changing `PotsCard.module.css:57` and `page.module.css:40` to `1fr 1fr` gives exit 2 with the
rule's message at both (so the glob reaches `app/(app)/`); the unit test is red without a config
(`ConfigurationError: No configuration provided`, 3 failed) and green with it (3 passed).
`npm run lint` becomes ESLint and then Stylelint, so CI's `lint · typecheck · unit` job and
`npm run test:all` run it with no workflow change and no new required check.
...
below (TD-7's two tests and fix; TD-8's option (b) and its three-engine run — the owner then chose (a);
TD-9's first design, a hand-written scanner, its fixtures and test, and on 2026-09-25 its rewrite
for Q4: `npm install --ignore-scripts --save-dev stylelint@17.15.0` (`package.json` and
`package-lock.json` edited and restored), `stylelint.config.mjs`, the new fixtures and test, the
`PotsCard` and `page.module.css` mutations, the regex mutation, a staged secret scan over the
lockfile; TD-10's functions, five test files, four wirings,
...
**The owner's answers, 2026-09-25** (their words: "Q1 - a, Q2 - tövsiyə olan, q3 - tövsiyyə olan,
q4 - linter, q5 - tövsiyyə olan, q6 - tövsiyyə olan, q7 - t-13 a/b plana daxildir ki, tövsiyyə
edirsən?"):

| Q   | Answer                                                    | Effect on this plan                                                                                                    |
| --- | --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Q1  | (a) fix the comment                                       | Task 2 as written; the owner waives the failing-first line for TD-8 (a comment cannot be red)                          |
| Q2  | as recommended: (a)                                       | Task 6 Steps 3–4 are unconditional: SPEC-reset-and-test-support v1.6 (§2.5, §2.7) approved; T-14 hand-off             |
| Q3  | as recommended: (a)                                       | as written                                                                                                             |
| Q4  | **a linter — not the recommended unit test**              | Task 3 rewritten (v0.2): Stylelint 17.15.0 as a dev dependency, `npm run lint:css` chained into `npm run lint`         |
| Q5  | as recommended: (A) fontsource static latin 400 and 700   | as written                                                                                                             |
| Q6  | as recommended: ESLint rule and the provenance test       | as written                                                                                                             |
| Q7  | a question back: "T-13a/b are in the plan, do you recommend it?" | answered under Q7 below — T-13a/b are **not** part of this plan; yes, the order and shape are recommended       |
| —   | execution method: not chosen yet                          | still open; recommended: subagent-driven                                                                               |

**Q1 — DECIDED (a). TD-8: fix the comment, or add the reverse walk? (Recommended: fix the comment.)** (a) The
...
**Q2 — DECIDED (a). TD-10 and T-14: how is Neon seeded the first time? (Recommended: (a).)** SPEC §2.5 names
...
**Q3 — DECIDED (a). TD-10: how wide is "deployment" and "test run"? (Recommended: (a).)** (a) Any value of
...
**Q4 — DECIDED: a linter, not the recommended unit test. TD-9: a unit test or a CSS linter?
(Recommended was the unit test.)** The recommendation was the unit test — one 50-line helper and
one 43-line test, no dependency — because a linter adds a dependency tree to `npm audit` (must stay
0) and to the `strict-allow-scripts` policy, plus a config, for one rule, and "choose the linter if
a second CSS rule is wanted soon". The owner chose the linter, so Task 3 is rewritten for
Stylelint, and what the choice costs was measured rather than assumed (2026-09-25, Node 26.7,
npm 11.19):

- `npm install --ignore-scripts --save-dev stylelint@17.15.0` adds **75 packages** and reports 0
  vulnerabilities; `npm audit --audit-level=high` exits 0 and `npm audit` finds 0. The lockfile grows
  by about 1 050 lines (a few dozen existing entries only move).
- `tests/unit/install-scripts.test.ts` (the `strict-allow-scripts` policy test) still passes: no
  package in the tree has an install script `allowScripts` does not name.
- The configuration is one file with **no `extends`** — no `stylelint-config-standard`, which would
  bring rules the existing CSS does not satisfy and a second dependency. One rule, one regular
  expression. A second CSS rule later is a new key in the same file.
- Stylelint runs in `npm run lint` (ESLint first, then `npm run lint:css`), which CI's `verify`
  job and `npm run test:all` already call: no workflow change, no new required check for T-16.
  `npm run lint:fix` is unchanged (this rule has no autofix).
- Vercel's `npm ci` installs dev dependencies too, so its install gets the 75 packages; the runtime
  bundle does not change. T-13d's dependency review (backlog row, item 4) sees one more
  dev dependency — the backlog changelog (Task 6) says so.
- No ADR is contradicted: only ADR-0002 names a lint tool (ESLint, for the import boundaries) —
  ADR-0001 and ADR-0003 do not mention one — and none closes the toolchain; `governance.md` has no
  rule on adding a dev dependency (checked 2026-09-25 with `grep`); the
  decision is recorded in the tech-debt entry, the backlog changelog and the process log, like the
  other owner decisions of this task.

**Q5 — DECIDED (A). TD-11: which font files? (Recommended: (A).)** (A) `@fontsource/public-sans` 5.3.0, static
...
**Q6 — DECIDED as recommended. TD-11: what checks the swap? (Recommended: the ESLint rule and the provenance test.)** The
...
**The owner's question on Q7, answered (2026-09-25):** "T-13a/b are in the plan — do you recommend
it?" **T-13a and T-13b are not part of this plan.** They are separate backlog rows with their own
plan gates (`middleware.ts` → `proxy.ts`; the `/_global-error` nonce), and on 2026-09-25 none has a
branch, a plan or a pull request; this plan neither includes nor blocks them. What the shape
recommendation says is only this: **yes, run T-13c on its own branch now, whatever the order of
the others.** Reasons, each checkable: (1) the backlog Notes make T-13c independent of both ("T-13b
after T-13a, since both touch the CSP path; T-13c is independent of both"); (2) the files this plan
changes — the File structure table — do not include `middleware.ts`, `proxy.ts`, the CSP builder
or `global-error`, so no code conflict is expected; (3) the three documents that will conflict
(`backlog.md`, `tech-debt.md`, `process-log.md`) are appended to, and are resolved by the rebase
that Task 6 Step 7 performs; (4) the one real ordering constraint runs the other way: **T-13d, the
security review, must come after T-13c is merged**, because it reads TD-10's guard and the font
change. If the owner would rather have T-13a/b first, nothing in this plan changes except the
rebase.
...
| `tests/e2e/auth-accessibility.spec.ts` (modify)                         | 2    | the walkthrough's comment                                                              |
| `stylelint.config.mjs` (create)                                         | 3    | the one rule: no bare `fr` column track                                                |
| `package.json`, `package-lock.json` (modify), `README.md` (modify)      | 3    | the `stylelint` dev dependency, `lint:css`, `lint` runs it; the scripts table          |
| `tests/fixtures/css-grid/*.css.fixture` (create, 2)                     | 3    | a file of violations, a file that must stay silent                                     |
| `tests/unit/css-grid.test.ts` (create)                                  | 3    | the real tree is clean; both fixtures behave — through the shipped config              |
...
**Open items for the owner.** (At v0.2:) The go-ahead ("start") and the execution method — both
came on 2026-09-25 (v0.3). Q1–Q6 are answered (the two spec edits inside Q2 are approved); Q7's
question is answered under Q7.
