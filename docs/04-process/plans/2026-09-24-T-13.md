...
found — see "Findings outside the task") and, after PR-A merges, `task/T-13-ci-hardening` (Tasks 1–11) ·
...
> `scripts/secret-scan.sh` and, for the TD-4 mutation, `LoginForm.tsx`/`SignupForm.tsx` — nothing
...
`Origin-Agent-Cluster: ?1` in `middleware.ts`, an API test pinning it, the adapter's `console.warn`
for a rejected registration (if Q2 says yes), a proposed ADR-0006 amendment (5) and a
SPEC-webmcp-tools v1.0.5 note. Its failing test is the API test plus the 16 red E2E runs above.
...
### Task A2: the adapter says why a registration failed (only if Q2 = yes)

**Files:**
- Modify: `src/webmcp/adapter.ts:121` (insert after the `aborted` return)
- Modify: `tests/unit/webmcp/adapter.test.ts` (one test in the `register` describe, after the
  "a registerTool rejection for one tool" test)

- [ ] **Step 1: Failing test.**

```ts
  it("warns — naming the tool and the reason — when a registerTool call is rejected", async () => {
...
```
  Run `npx vitest run tests/unit/webmcp/adapter.test.ts -t "warns"`. **Prediction:** fails, `toHaveBeenCalledTimes(1)` received 0.

- [ ] **Step 2: Implement.** In `register`, immediately after `if (controller.signal.aborted) return;`
  (the second one, line 121 — after `Promise.allSettled`), so a superseded generation, whose
  registrations the runtime rejects on purpose, stays silent:

```ts
  outcomes.forEach((outcome, index) => {
    if (outcome.status === "rejected") {
      console.warn(`[webmcp] could not register tool "${tools[index]?.name}"`, outcome.reason);
    }
  });
```

- [ ] **Step 3: Run** the whole adapter file. **Prediction:** all pass. `npm test` still 875 + 1.
- [ ] **Step 4: Commit** — `fix(webmcp): warn when a tool registration is rejected instead of swallowing it`.
...
- [ ] **Step 2:** SPEC-webmcp-tools — §2.2, after the `off` bullet: "The polyfill throws
  ...
  (ADR-0006 (5))." Add a `v1.0.5 (2026-09-24, T-13 plan finding F1)` entry to the changelog run in
...
**Q2 — Should a rejected registration warn in the console (Task A2)? (Recommended: yes, keep
`data-webmcp="ready"` as it is.)** The warn is 4 lines and makes the next F1 visible in a real
browser. Whether "ready" should also be withheld when *every* tool failed is a spec change
(§2.2), not made here.
...
| `src/webmcp/adapter.ts`, `tests/unit/webmcp/adapter.test.ts` | the rejected-registration warning | A2 |
...
- [ ] **Step 1: Branch and bootstrap.** After PR-A is merged (or, if the owner answered Q1 "no",
  from `origin/main`):
