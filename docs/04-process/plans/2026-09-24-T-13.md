...
Status: **Done** — executed as T-13 and merged (PR #29, 2026-09-24 UTC, merge `2e79edb`); until then it read **Draft — awaiting the owner's answers at the plan gate**, and the rest of this line and the text below are the plan as it stood at hand-over, not rewritten (v0.1 — 2026-09-24) · Author(s): Agent (Claude Code)
· Date: 2026-09-24
...
## Execution — what differed from this plan (2026-09-24)

Executed on 2026-09-24 with subagent-driven development (Sonnet 5 implementers, Opus 5.5
reviewers). PR-A = PR #28, merged (`99298f9`); T-13 = PR #29, merged (`2e79edb`; last commit `cc970a0`). The plan text
below is left as written — the plan gate's record, status line included — with the differences
listed here. Sources: the execution ledger and pre-flight scan (session-local, not committed) and
the per-task reports (`docs/04-process/prompts/2026-09-24-T-13/` on PR #29). SHAs are those on
`origin/main` (PR-A) and on PR #29 (T-13, after its rebase onto `99298f9`).

**PR-A (`fix/origin-agent-cluster`, PR #28)**

- **Numbers.** `webmcp.spec.ts` had 10 tests per engine at `838e0f5`, not 8: with the header 20
  passed (10 per engine), not the "16 passed (8 per engine)" of A1 Step 5. The plan's "16 red /
  8 failing per engine" is its own measurement and was never re-measured in PR-A; only the
  pass-count prediction was wrong. After A2 the file has 11 tests per engine (33 on three engines).
  A2's "override works on Firefox and WebKit" was measured on the headerless base (a prediction
  there); it passed with the header (3 passed).
- **A1 test strengthened** after review (`3b3d9d8`): the plan's verbatim test passed on `/overview`
  even when the login failed. It now asserts the login returns 200 and requests with
  `maxRedirects: 0`; shown failing on purpose with a wrong password (401).
- **A2 got a polish commit** (`38cd59c`): test-only, five review minors (a silent superseded
  generation, next-`register()` clearing, a polled console assertion, no `console.warn` noise, the
  describe title cites the SPEC); two mutation checks shown red.
- **A3's text grew** in a fix round after the PR-A whole-branch review (`f01458c`, `8d5cdef`,
  `ed68b73`): SPEC §2.2/§2.7 say `mode()` reports `unavailable` while every tool of the current
  generation is rejected, the warning text and reason format, and ADR-0006 (5) lists the
  alternatives not chosen; the process-log entry was corrected. The spec and ADR keep "proposed"
  wording — nothing was marked Approved or Accepted. A3 had no separate task review; the
  whole-branch review covered it. Its evidence: `npm run test:all` on `f01458c` exit 0 (secret
  scan 317 commits, unit 881 in 71 files, API 97, E2E on three engines 303 passed, 24 skipped, 0
  failed).

**T-13 tasks (`task/T-13-ci-hardening`, PR #29)**

- **Task 2** — a wiring test was added (`d627387`): no test pinned that the real coverage gate is
  wired (config thresholds equal the JSON, the glob matches files), so a zero-match glob would
  have passed silently. Also documents the fixtures in the READMEs.
- **Task 3** — the plan's text-regex scanner was replaced by a TypeScript-syntax-tree scanner
  (`c1087a7`; `typescript` is already a dev dependency): the regex counted titles inside comments,
  strings, template literals, skipped groups and method calls. One failing fixture per class, built
  from parts. A second round (`32ee66c`): the test's own title named US-41 (the file is scanned), and
  `ScriptKind` follows the file extension. The ADR-0003 clarification keeps the old sentence struck
  through, per the file's convention. Of the task-level fix rounds only these two had a scoped
  re-review; the other follow-ups were covered by the whole-branch review.
- **Task 4** — fixtures made two-way (`d464e47`): the empty-database fixture also passed against an
  unmigrated database, and a migration without a schema edit had no fixture (a `LoginAttempt`-dropped
  copy now expects exit 2).
- **Task 5** — the fixture is built from the DoD, not from the template, so it tests the comparison
  alone; the "all unticked" check has a fixture that fails on purpose, plus order and duplicate
  checks (`fda549a`, `32d3a8e`).
- **Task 6** — the route guard is two-way and the axe spec pins the page it scanned (status 200
  or 404 and the URL): a deleted-but-listed page would otherwise scan the 404 or login page under
  another name (`04dc381`); page discovery accepts `page.(tsx|ts|jsx|js)`. Axe: 24 passed on three
  engines; no real violation found, nothing suppressed.
- **Task 7** — the child environment filter is case-insensitive and the configs are hermetic
  (`~/.npmrc` cannot defeat the fixture); **`engines.npm >=11.19` was added, which the plan does
  not have** (`1f3099e`), because an older npm only warns `Unknown project config` and would
  silently not enforce the policy. The plan's eslint replace range covers mid-line 54 to 60, not
  53–60. Step 7 was done through Docker (`node:26`, npm 11.19.1): `npm ci` exit 0, and exit 1 with
  `ESTRICTALLOWSCRIPTS` naming `esbuild@0.28.2` when its entry is removed. Caveat: the flag was also
  passed on the command line, so `.npmrc` alone on Linux is covered only by the unit test.
- **Task 8** — three tracked docs contradicted the shipped behaviour (README, `scripts/README.md`,
  the secret-scan fixtures README) and were rewritten (`ebe0a15`); the script keeps its exit codes
  (the plan's `|| status=1` collapsed them), runs both scans and labels the output; the tag test
  asserts the rule id and the redaction (`9e19e33`). A commit-msg hook stays out of scope.
- **Task 9** — as planned; the mutation ran on Chromium only (4 failed), Firefox and WebKit were
  measured passing (8 passed), not mutation-tested. The PR number in TD-4 had no plan step: it
  was added after the PR opened (`7c807bf`). The evidence note tied the mutation to the wrong
  line and was reworded in Task 11 (line 78 is the network path, line 94 serves the 429).
- **Task 10** — 109 passed and 8 skipped per engine (117 tests), not 108; Firefox with
  `WEBMCP_MODE=off`: 106 passed, 11 skipped, 0 failed. Three wording fixes after review
  (`c8cf471`): four required leg names, the off rationale stated as a measurement not a proof, the
  WebKit note marked macOS. Linux legs were unmeasured until the first CI run.
- **Unit-test counts** drifted from the plan's 875-based predictions: 881 on `main` after PR-A,
  943 after the rebase, 958 at the T-13 head.

**Records and process**

- **Task 11** wrote a new "T-13 CI hardening — execution" entry on PR #29 instead of completing the
  planning entry, because the plan and its entry were not on `main`; this PR completes the planning
  entry. The plan file was not edited on the T-13 branch. It saved briefs and reports under
  `prompts/2026-09-24-T-13/` (not the ledger or the pre-flight scan). It also fixed two README
  lines that Tasks 2, 3 and 10 had made false, and re-measured the overrides: without them 4
  high advisories return (`deepmerge-ts`, `mysql2`); the newest stable Prisma 7.x is 7.10.0.
  A fix wave after the whole-branch review corrected the records (`bb059dc`, `82b8dc3`) and added
  three first-CI-run risks to the T-16 hand-off.
- **Pre-flight C1–C10** (the plan scanned against the tree before execution): wrong counts and
  line references (C1, C2, C4, C7), the eslint range and an unused README entry (C3), the plan
  living only on `worktree-t-13-plan` (C5), three branches appending to the process log (C6),
  Task 11's `test:all` needing PR-A (C8), a stale `childEnv` sentence (C9), TD-4's PR number
  having no step (C10). All ruled as corrections at execution; none blocked.
- **Rulings.** Q4 = B, Q5 = yes, Q7 = keep the overrides (removal note to T-16) and Q8 = deny
  `fsevents` were taken as the recommended answers (the owner said "recommended" for the four
  questions and then "start"), not confirmed one by one. Server and Postgres were used by one
  worktree at a time. T-13 was rebased onto `99298f9` without conflicts. This docs PR is a new branch
  from `origin/main` with the four plan commits cherry-picked; `worktree-t-13-plan` was not rebased.
