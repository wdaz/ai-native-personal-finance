...
Status: **Done** — executed as T-03 and merged (PR #8, 2026-09-22 UTC, merge `90440a1`); until then it read **Draft**, and the rest of this line and the text below are the plan as it stood at hand-over, not rewritten (v0.2 — the owner's plan-gate answers of 2026-09-22 applied; waiting for the go-ahead) · Author(s): Agent (Claude Code, Opus 5) · Date: 2026-09-22
...
1. ~~Answer questions 1–6 below.~~ Answered 2026-09-22: every recommendation, with conditions
   (§ "Owner answers at the plan gate"); applied in this v0.2.
2. Give the go-ahead. The owner chose "Bəli — yalnız plan v0.2" (plan v0.2 only): no execution
   before a separate go-ahead.
...
| E15 | **Not measured** | `npm run test:api` / `test:e2e` after Task 2 (needs Postgres in this worktree) — *prediction*: API 17/17, E2E 3/3 as after T-02, because `toCents` and `shiftYears` move verbatim. CI running `npm run seed:figures` (tsx) inside the `verify` job's Vitest — *prediction*: passes, tsx is a devDependency |
| E16 | v0.2, after the owner's answers 3 and 4: the five added tests (`formatMoney` 1 and −1 cent, `formatSignedMoney` 1 cent, `formatDate` 30 Sep 23:59:59Z and 31 Dec 23:59:59.999Z) in the prototype; `npx prettier --check`, `npx eslint`, `npx vitest run` | Prettier and ESLint clean; money **17**, dates **24**; the whole suite **340/340** |
...
- *Narrow the rule to the clock reads (recommended):* forbid `new Date()` without arguments,
  ...
  as local time, E6), not a clock read. It also lifts a guarantee T-02 relied on: T-02's decision
  D7 kept the seed's dates as text *because* the wide rule forbade parsing in `src/server`; after
  the change the `server-uses-new-date` fixture guards only the argument-less form, and nothing
  stops a later `src/server` task from parsing timestamps (T-02's code itself does not change).
...
whether it was the owner's plan-gate reply and how much of it to act on. The owner chose "Bəli —
yalnız plan v0.2" ("Yes — plan v0.2 only"): apply the answers to this plan, and start no
execution before a separate go-ahead. The reply, verbatim:
...
|---|----------|--------|---------------------------|
| 1 | Clock vs the lint rule | **Narrow the rule**: `new Date()` without arguments, `Date()`, `Date.now()` rejected; `new Date(<text>)` free. T-02's text dates (D7) were a side effect of the wide rule, not a goal; `shiftYears` is unchanged, so the behaviour stays, unguarded by lint — say so in ADR-0005 in one line: "parsing is allowed, reading the clock is not". Fixtures for three cases: `Date()` rejected, `Date.now()` rejected, `new Date("2026-…")` passes | Task 6 Step 2's clarification carries that sentence and the D7 note. Fixtures: `Date()` — `domain-calls-date`, `server-calls-date` (new); `new Date("2026-…")` — `domain-parses-date-allowed` (new; its second line is `new Date("2026-08-19T20:23:11Z")`); `Date.now()` — `domain-uses-date-now` and `server-uses-date-now`, **already in place since T-01** and re-run after the rule edit (Task 1 Step 8). A third `Date.now()` fixture would duplicate them, so none is added; the owner may ask for one |
| 2 | How seed-figures reads the seed | **Move** `toCents`, `shiftYears` verbatim; no re-export; only the tests' import paths change | Nothing — Task 2 already does exactly this |
| 3 | Who writes the §4.2 formatters | **T-03**, fully per §4.2, with boundary tests: 0, negative, 1 cent, December/September; T-04's scope narrows in backlog v1.8 | Task 4: `formatMoney` gains `1 → $0.01` and `-1 → -$0.01`, `formatSignedMoney` gains `1 → +$0.01`; `formatDate` gains a September test and a 31 December 23:59:59.999Z test (E16). Task 6 Step 1 narrows T-04's row |
| 4 | "Sep" or "Sept" | **"Sep"**: a fixed three-letter table (`Jan … Dec`), no `Intl`; §4.2's `Intl` example replaced by the table; UTC stays; a separate September test | Task 5 Step 5's §4.2 row lists the twelve names; Task 4's September test (E16) |
| 5 | §4.3 per-budget amounts | **`$15.00`** | Nothing — Task 5 Step 5 |
| 6 | Document changes | **All in this PR**; the PR description lists the three changes separately | Task 6 Step 5 names them |
| F1 | `Date()` gap | Fixed in Task 1; no separate PR | Nothing |
| F2 | Coverage globs | **T-13**; do not widen this PR | Recorded in T-13's backlog row (Task 6 Step 1), no config change here |
| F3 | Wall clock for T-05 | **T-05**, whose plan must separate "system time" from the "business Clock" | Recorded in T-05's backlog row (Task 6 Step 1) and in the hand-off notes |
...
- **T-05:** F3 — the plan must separate **system time** (the session's sliding re-issue, the rate
  limit's window: real instants) from the **business `Clock`** (a fixed calendar day for business
  rules), and inject system time into `src/server` rather than read it there (owner, F3). The
  ADR-0005 rule now allows `new Date(<value>)` and still forbids `new Date()`, `Date()` and
  `Date.now()` (question 1).
...
- **Release 2:** `sortTransactions` reuses `compareLatest` (US-11); the recurring-bills page uses the
  ...
  US-27 AC2's "day-of-month ≤ today + 5" has no month-end case: from the 27th, today + 5 passes
  the month's last day and every unpaid bill reads Due Soon. Unreachable while business time is
  the 19th, and `recurringBills` implements the rule as written — the gap is the story's, for the
  Release 2 spec to settle.
...
| `tests/unit/shared/money.test.ts`, `dates.test.ts` | 17 + 24 tests | 4 |
...
229 → **244** → **263** → **291** → **332** → **340** (v0.2: +5 boundary tests of question 3, E16).
