...
Status: **Draft** (v0.2 — owner's plan-gate answers of 2026-09-22 applied; question 2 open) · Author(s): Agent (Claude Code, Opus 5) · Date: 2026-09-22
...
| 2 | Keep the `docs/00-discovery/inputs/` path allowlist? | **Open** — the question was unclear; re-asked in plain terms | Nothing yet. Until the owner answers, the plan keeps the approved backlog wording (both `.next/` and `docs/00-discovery/inputs/` exempt) |
...
wait.) The agent applied 1, 3, 4 and 5 to the plan (v0.2) and re-asked question 2.

<every later reply, verbatim with its date: the answer to question 2 and the go-ahead>
```

Before committing, replace the last angle-bracket line with the owner's later replies,
verbatim.
...
- **Owner decisions at the plan gate:** T-16 names the `secret scan` check and
  ...
  question 2 (the `docs/00-discovery/inputs/` allowlist) — <the owner's answer>.
