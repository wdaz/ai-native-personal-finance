...
Status: **Draft** · Author(s): Agent (Claude Code, Opus 5) · Date: 2026-09-22
...
- **Out of scope:** T-02 (Prisma, Docker compose, any real `DATABASE_URL`); T-13 (CI
  triggers on `main` only, the `concurrency` split, the install-script policy); T-16
  (rotation, the public flip, GitHub secret scanning / push protection, CodeQL,
...
| # | Measurement | Result |
|---|-------------|--------|
| E1 | `gh release view -R gitleaks/gitleaks` | Latest is **v8.30.1**, published 2026-03-21 |
...
| D11 | CI gets two jobs: `secret scan` (checkout with `fetch-depth: 0`, `persist-credentials: false`, then the script — no Node, no `npm ci`) and `npm audit` (setup-node, then `npm audit --audit-level=high`, blocking; it reads `package-lock.json`, so no `npm ci`) | Separate check names, independent verdicts, no install time spent | Both as steps of the existing `verify` job |
| D12 | `npm run secrets:scan` becomes the first step of `test:all`; `npm audit` does not | ADR-0003: `test:all` is what CI runs. The audit's verdict depends on the registry's advisory feed at that moment, not on the code | Leaving the scan out of `test:all` |
...
1. **T-16 inherits the merge blind spot unless told.** T-16 says "full-history secret scan
   as a blocking gate". After T-02a the CI job already scans the full history, merges
   included, on every pull request; what T-16 still needs is to make `secret scan` (and
   `npm audit`, if wanted) a *required* check in the `main` ruleset, and to run
   `npm run secrets:scan` rather than a bare `gitleaks git`, which skips the six merge
   commits (E3). Would you add a line to the T-16 row? This plan edits no backlog.
2. **The two path allowlists** are implemented as approved. Today neither produces a
   finding (a `dir` scan of the working tree with the default rules finds nothing, and
   `.next/` does not exist until a build). A path allowlist is a hole; keep
   `docs/00-discovery/inputs/`?
3. **Out of scope, noticed:** `.github/workflows/ci.yml` still triggers on
   `branches: [main, master]` under a comment saying the default branch is `master` —
   stale since the migration. T-13 owns the triggers, so this plan leaves it.
4. **`npm audit` blocks.** A newly published advisory will turn CI red on an unrelated pull
   request. The remedy then is to update the dependency or add an `overrides` entry,
   recorded in that PR. Agreed?
5. **Prompt file.** No task prompt existed for T-02a; the session began with
   `/superpowers:writing-plans t-02a başlayaq`. Task 5 saves that as
   `docs/04-process/prompts/2026-09-22-T-02a-secret-guard.md`, together with your reply to
   this plan.
...
| `.github/workflows/ci.yml` | Modified: jobs `secret-scan` ("secret scan") and `audit` ("npm audit") |
...
Run from the repository root, on branch `task/T-02a-secret-guard` (the branch that carries
this plan):

```bash
git switch task/T-02a-secret-guard
npm ci
```

Expected: `npm ci` completes (`found 0 vulnerabilities`). There is no `prepare` script yet.
...
- Produces: pull-request checks named **`secret scan`** and **`npm audit`**, next to the
  existing `lint · typecheck · unit`.

- [ ] **Step 1: Add the two jobs**

Append to `.github/workflows/ci.yml`, under `jobs:`, after the `verify` job (leave the
`on:` block and `concurrency` untouched — T-13):
...
```yaml
  ...
  audit:
    ...
    steps:
      ...
      # Reads package-lock.json against the registry's advisories; high or critical fails
      # the job. The verdict can change without a code change, when an advisory is published.
...
ruby -ryaml -e 'y = YAML.load_file(".github/workflows/ci.yml"); p y["jobs"].keys; p y["jobs"]["secret-scan"]["steps"][0]["with"]'
```

Expected: Prettier clean; `["verify", "secret-scan", "audit"]` and
...
tests plus these 19, `test:api` with no tests, E2E on three engines. If Playwright
...
git commit -m "ci(secret-guard): secret scan and npm audit jobs; run instructions (T-02a)
...
fails on high and critical advisories. README and scripts/README describe the hook and
...
the `secret scan` log: `N commits scanned` and `no leaks found`. The `verify` job's first
unit run downloads gitleaks on the runner.
...
## Owner's reply at the plan gate

<paste the owner's reply verbatim, including any decision D1–D15 overturned and the
answers to questions 1–5>
```

Replace the angle-bracket line with the owner's actual reply before committing.

- [ ] **Step 2: Append the process-log entry**
...
- **Disagreements:** during planning, a reviewer pass proposed path-allowlisting the
  ...
  fixtures scan clean and the materialised ones fire on every line.
- **Lessons for the process:** a scanner's defaults are part of what it guarantees —
  ...
  test.
- **Next:** owner review and merge; T-02 (the first real `DATABASE_URL`); owner decision on
  plan question 1 (T-16 and the merge-commit scan).
```

- [ ] **Step 3: Commit and push**

```bash
git add docs/04-process/prompts/2026-09-22-T-02a-secret-guard.md docs/04-process/process-log.md
# subagent-driven execution only:
git add docs/04-process/prompts/2026-09-22-T-02a
git commit -m "docs(process): T-02a prompt and process-log entry

Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>"
git push
```

- [ ] **Step 4: Write the Definition of Done into the pull request**
...
- [x] Names the task, spec sections and stories; nothing outside T-02a changed (the stale
  `master` trigger in `ci.yml` is T-13's, left as is — plan question 3).
- [x] No Accepted ADR contradicted — ADR-0007's CI list gains two jobs; ADR-0002's
  `scripts/` holds the new tooling.
- [x] No spec amended. The merge-commit finding affects T-16's wording; raised as plan
  question 1 for the owner.
...
- [ ] **Step 5: Mark the PR ready and stop**
