...
Status: **Approved** (v1.0, owner approval 2026-09-13) · Author(s): Agent · Date: 2026-09-13 · Traces to: user stories US-04…US-30, US-36/37; ADR-0005
Money is integer cents. Dates are UTC timestamps; business time is fixed (ADR-0005). Every entity has a UUID `id`, `createdAt`, `updatedAt`, `seeded` (boolean).
...
| `Budget` | `category` (enum, **unique**), `maximum` (cents > 0), `theme` (enum, 15, **unique among budgets**) | spent/remaining are computed, not stored | US-07, US-14–20 |
| `Pot` | `name` (≤ 30, **unique, case-insensitive**), `target` (cents > 0), `total` (cents ≥ 0), `theme` (enum, **unique among pots**) | `total` may exceed `target`; deposit ≤ `Balance.current`; withdrawal ≤ `total` | US-05, US-21–26 |
