...
- Status: **Accepted** · Date: 2026-09-13 · Author(s): Agent, Owner (constraint: public deploy, all browsers)
...
- **CI (GitHub Actions):** on PR — install, lint, typecheck, unit + component, API tests and E2E against a local Postgres service container with `next build && next start`, axe, traceability script; on `main` — the same plus visual snapshots and Lighthouse CI against the production URL after deploy (warm-up request first).
