...
describe("evaluateAttempts", () => {
  it("lets the 10th failed attempt through — 9 prior failures, not yet limited", () => {
    const result = evaluateAttempts(9, now, new Date(now.getTime() - 60_000));
    expect(result.limited).toBe(false);
  });

  it("blocks the 11th attempt — 10 prior failures already in the window", () => {
  ...
  it("retryAfter counts from the oldest counted failure, rounded up to whole seconds", () => {
    ...
    const result = evaluateAttempts(10, now, oldest);
